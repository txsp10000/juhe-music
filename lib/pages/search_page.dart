...
class _SearchPageState extends State<SearchPage> {
    ...
    return Scaffold(
      ...
      body: MusicScaffoldBackground(
        ...
        child: SafeArea(
          bottom: false,
          child: ListView(
            padding: EdgeInsets.fromLTRB(20, 18, 20, widget.embedded ? 106 : 24),
            children: [
              _buildHeader(),
  ...
  Widget _buildSearchBar() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white.withOpacity(0.10), borderRadius: BorderRadius.circular(26)),
      child: Row(
        children: [
          Icon(Icons.search_rounded, color: AppDesignTokens.warmWhite.withOpacity(0.65), size: 25),
          const SizedBox(width: 10),
          Expanded(
            child: TextField(
          ...
          if (_controller.text.isNotEmpty)
            GestureDetector(onTap: () => _controller.clear(), child: Icon(Icons.close_rounded, color: AppDesignTokens.warmWhite.withOpacity(0.65), size: 20)),
        ],
      ),
    );
  }
  ...
  Widget _buildHistory() {
    return Column(
      ...
      children: [
        ...
        if (_history.isEmpty)
          Text('暂无搜索历史', style: AppDesignTokens.body(color: AppDesignTokens.warmWhite.withOpacity(0.55)))
        else
          Wrap(
            spacing: 10,
            runSpacing: 10,
  ...
  Widget _historyChip(String kw) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          GestureDetector(onTap: () => _doSearch(kw), child: Text(kw, style: AppDesignTokens.body(size: 14, weight: FontWeight.w800))),
          const SizedBox(width: 8),
          GestureDetector(onTap: () => _removeHistory(kw), child: Icon(Icons.close_rounded, color: AppDesignTokens.warmWhite.withOpacity(0.60), size: 16)),
        ],
      ),
    );
  }
